"""Re-emit Pipeline-1 Stream 2 features with Hermite coefficients (stage B).

Stream-2 analogue of ``scripts/emit_stream1_with_hermite.py`` — same Hermite
...
.provenance.json``) at assembly time — Stream-2 MUST use training-set stats
verbatim, not refit its own.

Outputs
-------

Same path (``data/processed/pipeline1_features_stream2.parquet``) with
...
logger = logging.getLogger("emit_stream2_hermite")
...
def main() -> None:
    ...
    src = repo / "data" / "processed" / "pipeline1_features_stream2.parquet"
    dst = src
    decisions_path = (
    ...
    prov = Provenance(
        ...
        script="scripts/emit_stream2_with_hermite.py",
        sources=[
            LocalSource(
                name="Pipeline-1 Stream-2 stage-A features (Ye-corrected sampled flux)",
        ...
        notes=(
            "Stage B of Stream-2 emit — raw schema. Produces bp_coef_norm_{i} as "
            ...
            "extra.c0_zscore_frozen). This guarantees Stream-2 uses the exact "
