"""Gaia DR3 enrichment for Stream 3 — Andrae+2023 RGB stratified sample.

Mirrors ``fetch_gaia_enrich_stream1.py`` but against the 168 k Stream 3
source_ids in ``interim/stream3_selected_source_ids.parquet``. Uses the
same TAP UPLOAD + JOIN pattern — inline ``IN`` clauses 504 AIP's gateway
above ~5 k IDs (see memory/reference_aip_tap_upload.md).

Expected wall-time: ~17 batches × ~20 s/batch ≈ 6 min.

Output
------
``data/interim/stream3_gaia_dr3_raw.parquet`` with the §3.6 enrichment
columns + provenance sidecar.
"""
...
logger = logging.getLogger("fetch_gaia_enrich_stream3")
...
def main() -> None:
    ...
    out = repo / "data" / "interim" / "stream3_gaia_dr3_raw.parquet"
    ckpt = repo / "data" / "interim" / "enrich_batches" / "stream3_gaia"
    ...
    df = batched_upload_fetch_df(
        ...
        queue="2h",
        runid="arqueogal-stream3-enrich",
    )
    ...
    prov = Provenance(
        ...
        script="scripts/fetch_gaia_enrich_stream3.py",
        sources=[
            LocalSource(
                name="Stream 3 selected source_ids (Andrae+2023 stratified)",
