...
def panel_01_data_flow() -> None:
    ...
    def box(x, y, w, h, text, color, fontsize=9, weight="normal"):
        patch = FancyBboxPatch(
            (x, y), w, h, boxstyle="round,pad=0.08", linewidth=1.2,
...
def panel_02_sky_mollweide(stream1_sample: int, stream3_full_sample: int) -> None:
    ...
    specs = [
        ("Stream 1 — APOGEE DR19 × Gaia (training)", STREAM1_FEATURES,
         ["ra_deg", "dec_deg"], None, "#1f77b4", stream1_sample),
        ("Stream 2 — TESS Hon+2021 × Gaia (pre-staged)", STREAM2_GAIA,
         ["ra", "dec"], None, "#ff7f0e", None),
        ("Stream 3 — selected RGB+RC subsample (inference)", STREAM3_SELECTED,
         ["ra_deg", "dec_deg"], None, "#2ca02c", None),
        ("Andrae+2023 RGB+RC parent sample (10.5M)", ANDRAE_FULL,
         ["ra_deg", "dec_deg"], None, "#7f7f7f", stream3_full_sample),
    ]
    ...
    for idx, (title, path, cols, _, color, sub) in enumerate(specs, start=1):
        ...
        ax.scatter(-lon_rad, lat_rad, s=0.5, c=color, alpha=0.25,
                   linewidths=0, rasterized=True)
        ax.set_title(f"{title}\nN={len(df):,}", fontsize=10)
        ax.grid(True, alpha=0.3)
        _apply_astro_mollweide_convention(ax)

    fig.suptitle(r"Sky footprint — galactic coordinates $(\ell, b)$",
                 fontsize=11, y=0.98)
...
def panel_03_kiel() -> None:
    ...
    sc = ax.scatter(df["teff_apogee"], df["logg_apogee"], c=df["mh_apogee"],
                    s=2, alpha=0.35, cmap="viridis", vmin=-1.5, vmax=0.5,
                    rasterized=True)
    ax.add_patch(plt.Rectangle((4000, 1.0), 1500, 2.5,
                               fill=False, edgecolor="red", linewidth=1.8,
                               linestyle="--", label="RGB+RC selection window"))
    ax.invert_xaxis()
    ax.invert_yaxis()
...
def panel_04_tinsley_wallerstein() -> None:
    df = pd.read_parquet(STREAM1_FEATURES,
                         columns=["fe_h_apogee", "alpha_m_apogee", "mh_apogee"])
    df = df.dropna(subset=["fe_h_apogee", "alpha_m_apogee"])
    fig, ax = plt.subplots(figsize=FIGSIZE_SQUARE)
    hb = ax.hexbin(df["fe_h_apogee"], df["alpha_m_apogee"],
                   gridsize=120, mincnt=3, cmap="magma_r",
...
def panel_05_magnitude_hist() -> None:
    ...
    if STREAM1_FEATURES.exists():
        ...
        ax.hist(g1, bins=np.linspace(6, 20, 141), histtype="step",
                linewidth=1.8, color="#1f77b4",
                label=f"Stream 1 (APOGEE DR19 × Gaia), N={len(g1):,}")

    if STREAM2_GAIA.exists():
        g2 = pd.read_parquet(STREAM2_GAIA, columns=["phot_g_mean_mag_corr"])["phot_g_mean_mag_corr"].dropna()
        ax.hist(g2, bins=np.linspace(6, 20, 141), histtype="step",
    ...
    if STREAM3_SELECTED.exists():
        ...
        ax.hist(g3, bins=np.linspace(6, 20, 141), histtype="step",
                linewidth=1.8, color="#2ca02c",
                label=f"Stream 3 (selected RGB+RC), N={len(g3):,}")

    ax.axvline(17.65, color="red", linestyle="--", linewidth=1.2,
...
def panel_06_label_matrix() -> None:
    tiers = [
        ...
        ("Tier 3\n(not released per-star)",
         ["si_h_apogee", "ca_h_apogee", "ti_h_apogee", "mn_h_apogee",
          "ni_h_apogee", "na_h_apogee", "cr_h_apogee", "k_h_apogee",
          "v_h_apogee", "s_h_apogee", "ce_h_apogee", "c_h_apogee",
          "n_h_apogee", "o_h_apogee"]),
    ]
    ...
    for i0, i1, name in tier_boundaries:
        ax.axvspan(i0 - 0.45, i1 + 0.45, alpha=0.08,
                   color="blue" if "Tier 1" in name else "orange" if "Tier 2" in name else "gray")
        ax.text((i0 + i1) / 2, 115, name, ha="center", va="center",
                fontsize=9, weight="bold")

    ax.set_title(f"Stream 1 label availability  —  {n_ok:,} flag_bad==0 rows\n"
...
def panel_07_extinction_priors() -> None:
    ...
    ax = axes[0]
    ax.bar(range(len(cols)), list(coverage.values()),
           color=["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd"],
           edgecolor="black", linewidth=0.5)
    ax.set_xticks(range(len(cols)))
    ax.set_xticklabels([c.replace("av_", "").replace("ag_", "")
                        for c in cols], rotation=20, ha="right")
    ...
    for c, color in zip(cols, ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd"]):
        ...
        if len(vals):
            ...
            ax.hist(vals, bins=bins, histtype="step", linewidth=1.4,
                    label=lab, color=color, density=True)
...
def panel_08_rowcount_waterfall() -> None:
    ...
    for bar, c in zip(bars, counts):
        ax.text(bar.get_x() + bar.get_width() / 2, c + 10_000,
                f"{c:,}", ha="center", fontsize=9)
    ax.set_xticks(xs)
    ax.set_xticklabels(labels, rotation=18, ha="right", fontsize=9)
    ax.set_ylabel("row count")
    ax.set_title("Stream 1 waterfall — APOGEE DR19 → training matrix\n"
...
def main() -> None:
    ...
    parser.add_argument("--stream1-sample", type=int, default=0,
                        help="Random sub-sample for Stream 1 sky plot (0 = all).")
    parser.add_argument("--stream3-full-sample", type=int, default=200_000,
                        help="Random sub-sample for Andrae+2023 full RGB+RC sky plot.")
    parser.add_argument("--skip", nargs="*", default=[],
                        help="Panel numbers to skip (e.g. --skip 2 8).")
    ...
    panels = {
        ...
        "2": lambda: panel_02_sky_mollweide(args.stream1_sample or None,
                                            args.stream3_full_sample),
        "3": panel_03_kiel,
