...
def _draw_rings_face(ax, x_lo, x_hi, y_lo, y_hi, *, origin=(0.0, 0.0)):
    ...
    for R_ref, color, label in RINGS:
        ...
        if in_box.any():
            ax.plot(cx[in_box], cy[in_box], color=color, lw=1.0, ls="--", alpha=0.85, label=label)
...
def main() -> None:
    ...
    pad_xgc = 0.05 * (x_gc_hi - x_gc_lo)
    pad_ygc = 0.05 * (y_gc_hi - y_gc_lo)
    x_gc_lo -= pad_xgc
    x_gc_hi += pad_xgc
    y_gc_lo -= pad_ygc
    y_gc_hi += pad_ygc
    ...
    pad_xh = 0.05 * (x_h_hi - x_h_lo)
    pad_yh = 0.05 * (y_h_hi - y_h_lo)
    x_h_lo -= pad_xh
    x_h_hi += pad_xh
    y_h_lo -= pad_yh
    y_h_hi += pad_yh
    ...
    axes[0, 0].scatter(
        [0], [0], marker="x", s=130, color="red", lw=2.6, zorder=10, label="GC (R=0)"
    )
    ...
    _draw_rings_face(axes[0, 1], x_gc_lo, x_gc_hi, y_gc_lo, y_gc_hi, origin=(0.0, 0.0))
    axes[0, 1].scatter(
        [-R_SUN],
        [0],
        marker="*",
    ...
    axes[0, 1].scatter(
        [0], [0], marker="x", s=140, color="red", lw=2.6, zorder=10, label="GC (0, 0)"
    )
    ...
    for r_ref, color, label in [
        (1.0, "#9467bd", "r=1 kpc"),
        (2.0, "orange", "r=2 kpc (typical)"),
        (5.0, "#1f77b4", "r=5 kpc"),
    ]:
        if r_ref <= max(r_h_max, R_SUN + 0.5):
            axes[1, 0].axvline(r_ref, color=color, lw=0.9, ls="--", alpha=0.85, label=label)
    ...
    for r_ref, color, label in [
        ...
        if in_box.any():
            axes[1, 1].plot(
                cx[in_box], cy[in_box], color=color, lw=1.0, ls="--", alpha=0.85, label=label
            )
    axes[1, 1].scatter(
        [0],
        [0],
    ...
    axes[1, 1].scatter(
        [R_SUN], [0], marker="x", s=140, color="red", lw=2.6, zorder=10, label="GC ($R_\\odot$, 0)"
    )
    ...
    if s2_path.exists():
        ...
        if "ra_deg" in s2_full and "r_med_photogeo" in s2_full:
            s2 = pd.read_parquet(s2_path, columns=["r_med_photogeo", "ra_deg", "dec_deg"])
            sky2 = SkyCoord(
                ra=s2["ra_deg"].to_numpy() * u.deg,
            ...
            scatter_kw = dict(s=1.2, color="#9467bd", alpha=0.45, rasterized=True, zorder=8)
