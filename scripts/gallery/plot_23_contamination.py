...
XB = np.linspace(-2.5, 0.6, 64); YB = np.linspace(-0.2, 0.5, 64)
...
def _hell(a, b): return float(np.sqrt(0.5 * np.sum((np.sqrt(a) - np.sqrt(b))**2)))
def _tv(a, b): return float(0.5 * np.sum(np.abs(a - b)))
...
def main() -> None:
    ...
    Y_pool = Y[tri]
    splits = (("Train (LOO)", Y[tri], np.median(Y_pool[idx_tr], axis=1)),
    ...
    for r, (name, Yt, Yp) in enumerate(splits):
        ...
        for k in range(3):
            ...
            ht /= max(ht.sum(), 1); hp /= max(hp.sum(), 1)
            hell.append(_hell(ht, hp)); tv.append(_tv(ht, hp))
        ...
        ax.set_xticks([0, 1, 2]); ax.set_xticklabels(["G1", "G2", "G3"], fontsize=8)
        ax.set_yticks([0, 1, 2]); ax.set_yticklabels(["G1", "G2", "G3"], fontsize=8)
        ax.set_xlabel("predicted"); ax.set_ylabel(f"{name}\ntruth")
        if r == 0: ax.set_title("Confusion matrix")
        ...
        for j in range(3):
            bars = ax.bar(["G1", "G2", "G3"], flow_pct[:, j], 0.6, bottom=bottoms,
                            color=pal[j], label=f"→ pred G{j+1}", edgecolor="white", lw=0.4)
            for i, b in enumerate(bars):
                if flow_pct[i, j] > 5:
                    ax.text(b.get_x() + b.get_width() / 2, b.get_y() + b.get_height() / 2,
                             f"{flow_pct[i, j]:.1f}%", ha="center", va="center",
                             fontsize=6, color="white")
            bottoms += flow_pct[:, j]
        ax.set_ylabel("% of truth class"); ax.set_xlabel("truth")
        if r == 0:
        ...
        ax.set_xticks(x); ax.set_xticklabels(["G1", "G2", "G3"])
        ax.set_ylim(0, 1.0); ax.set_ylabel("distance")
        if r == 0:
