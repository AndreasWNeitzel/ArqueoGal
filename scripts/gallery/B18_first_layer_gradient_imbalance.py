...
def main() -> int:
    ...
    ax.set_title(
        "Pre-scaling: per-column log10 median |x| - "
    ...
    ax.set_ylabel(r"$\log_{10}$ median$|x_i|$")
    ax.set_title(
        "Post-scaling: per-column log10 median |x| - "
    ...
    ax.set_xlabel("Feature index (encoder input order)")
    ax.set_ylabel(
        r"$\log_{10}\,(\mathrm{pre}/\mathrm{post})$ "
        r"$=$ gradient-imbalance reduction factor"
    )
    ax.set_title(
        "Imbalance reduction factor per column "
    ...
    fig.suptitle(
        ...
        "so per-column |x| sets per-column gradient scale.\n"
        "Pre-scaling, columns range over ~50 orders of magnitude; "
        "post-scaling, all O(1).",
        fontsize=11,
        fontweight="semibold",
        y=0.995,
    )
    ...
    save_fig(fig, OUT / "B18_first_layer_gradient_imbalance", formats=("pdf", "png"))
    return 0
