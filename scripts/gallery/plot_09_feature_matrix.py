...
AUX_COLS = [
    ...
    "bp_c0_z", "rp_c0_z",
    # residuals
    "reprojection_residual_rms", "reprojection_residual_rms_bp", "reprojection_residual_rms_rp",
    # astrometry + quality
    ...
    # extinction block
    "av_edenhofer", "av_sfd", "av_lallement", "av_nbhd_median",
    "av_nbhd_std", "n_neighbors_75pc",
    "ag_gspphot", "ebv_edenhofer_2023", "ebv_sfd",
    # flags
    "ye2024_flag", "xp_fit_flag_residual_high", "xp_fit_flag_residual_high_global",
...
def feature_layout_schema() -> None:
    ...
    for x0, x1, lbl, color in segments:
        ax.add_patch(Rectangle((x0, 0), x1 - x0, 1.0, facecolor=color, edgecolor="#222",
                               lw=0.7, alpha=0.85))
        ax.text((x0 + x1) / 2, 0.5, lbl, ha="center", va="center",
                fontsize=10, color="white", fontweight="semibold")
        ax.text((x0 + x1) / 2, 1.15, f"{x1 - x0} dims", ha="center", va="bottom",
                fontsize=9, color="#333")

    ax.set_xlabel("input-tensor column index")
    ax.set_title(r"Pipeline-1 NN input layout  —  153 dims  =  108 XP shape  +  2 $c_0$ scalars  "
...
def feature_distributions_xp() -> None:
    ...
    for i, ax in enumerate(axes.flat):
        ...
        ax.hist(vals_c, bins=np.linspace(-5, 5, 40),
                color="#1f77b4", alpha=0.8, density=True, edgecolor="#333", linewidth=0.3)
        # reference Gaussian
    ...
    fig.suptitle(r"Pipeline-1 XP feature distributions  —  110 post-z-score Hermite coefs "
...
def feature_distributions_aux() -> None:
    ...
    for i, ax in enumerate(axes.flat):
        ...
        if is_flag and col != "n_neighbors_75pc":
            ...
            ax.hist(np.clip(v, lo, hi), bins=np.linspace(lo, hi, 36),
                    color="#9467bd", alpha=0.8, edgecolor="#333", linewidth=0.3)
        pct_nan = 100.0 * df[col].isna().mean()
        ax.set_title(f"{col}\nn={len(v):,}  NaN={pct_nan:.1f}%", fontsize=8, pad=2)
        ax.tick_params(axis="both", labelsize=7)

    fig.suptitle("Pipeline-1 aux feature distributions  —  Gaia + IR + extinction + flags",
                 fontsize=13, fontweight="bold", y=0.995)
    fig.tight_layout(rect=(0, 0, 1, 0.975))
...
def feature_correlation_heatmap() -> None:
    ...
    ax.set_yticks(ticks); ax.set_yticklabels(labels, fontsize=10, rotation=90, va="center")
    ax.set_title(f"Pipeline-1 {len(cols)}×{len(cols)} input-feature correlation matrix "
