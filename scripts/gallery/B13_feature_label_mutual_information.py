...
def main() -> int:
    ...
    print(
        f"[B13] metal-poor cohort: high-α n={int(high_alpha_mp.sum()):,}, "
    ...
    print(
        f"[B13] full cohort:        high-α n={int(high_alpha_all.sum()):,}, "
    ...
    ax.set_title(
        f"|Cohen's d| per encoder feature, metal-poor "
    ...
    ax.barh(np.arange(20)[::-1], d_mp[order], color=[family_color[family[i]] for i in order])
    ...
    ax.set_ylabel("|Cohen's d|")
    ax.set_title(
        f"|Cohen's d| per encoder feature, FULL Kiel pool, high-α vs low-α at [α/M] = {alpha_thr}."
    )
    ax.axhline(0.1, color="k", lw=0.6, ls=":", alpha=0.5)
    ax.axhline(0.5, color="r", lw=0.6, ls="--", alpha=0.6)
    ax.grid(axis="y", alpha=0.25)
    ...
    save_fig(fig, OUT / "B13_feature_label_mutual_information", formats=("pdf", "png"))
    return 0
