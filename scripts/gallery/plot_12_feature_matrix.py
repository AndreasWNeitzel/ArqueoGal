...
def main() -> None:
    ...
    ncol = 4
    nrow = (n + ncol - 1) // ncol
    fig, axes = plt.subplots(nrow, ncol, figsize=(15, 2.6 * nrow), constrained_layout=True)
    flat = axes.flatten()
    for i, c in enumerate(feats):
        ...
        for name, color, df in loaded:
            ...
            if len(v) < 10:
                continue
            ax.hist(v, bins=bins, density=True, histtype="step", color=color, lw=1.2, label=name)
    ...
    fig.suptitle("Stage 12 — aux feature distributions across S1 / S2 / S3 (overlay)", fontsize=11)
