...
def main() -> int:
    ...
    if not feat_path.exists() or not pred_path.exists():
        print(f"Error: required inputs missing.\n  features: {feat_path}\n  predictions: {pred_path}")
        return 1
    ...
    preds = pd.read_parquet(
        pred_path,
        columns=["source_id", "teff_pred", "logg_pred",
    ...
    print(f"[C2] split sizes: train={len(splits['train']):,}, "
    ...
    save_fig(fig, out_dir / "C2_train_val_test_predictions",
             formats=("pdf", "png"))
    return 0
