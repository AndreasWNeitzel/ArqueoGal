...
def finetune_loss_curve() -> None:
    ...
    if path is None:
        ax.text(0.5, 0.5, "finetune history not found", ha="center", va="center",
                fontsize=12, color="#d62728", transform=ax.transAxes)
        save_fig(fig, OUT / "finetune_loss_curve.png"); return
    ...
    best = h.get("best_epoch"); best_val = h.get("best_val_loss")
    if best is not None and best_val is not None:
        ax.axvline(best, color="#333", lw=0.8, ls=":",
                   label=f"best epoch {best} (val={best_val:.4f})")
    ax.set_xlabel("epoch"); ax.set_ylabel(r"beta-NLL loss ($\beta=0.5$)")
    ax.set_title(f"5-label supervised fine-tune  —  {path.parent.name}/{path.name}",
...
def ensemble_member_spread() -> None:
    ...
    if path is None:
        ax.text(0.5, 0.5, "ensemble history not found", ha="center", va="center",
                fontsize=12, color="#d62728", transform=ax.transAxes)
        save_fig(fig, OUT / "ensemble_member_spread.png"); return
    h = _load_json(path)
    ...
    for i, m in enumerate(members):
        ...
        if isinstance(hist, list) and len(hist):
            ...
            ax.plot(eps, va, "-", color=colors[i % 10], lw=1.2,
                    label=f"seed {m.get('seed', i)}")
            any_plotted = True
    if h.get("val_loss_mean") is not None:
        ax.axhline(h["val_loss_mean"], color="k", lw=0.7, ls="--",
                   label=f"mean best val = {h['val_loss_mean']:.4f} "
                         f"(spread={h.get('val_loss_spread', 0):.4f})")
    ax.set_xlabel("epoch"); ax.set_ylabel("val loss")
    ax.set_title(f"5-label ensemble — per-member val-loss curves  ({len(members)} members)",
                 fontsize=11, fontweight="semibold")
    if any_plotted:
...
def lr_schedule() -> None:
    ...
    if path is None:
        ax.text(0.5, 0.5, "no history", ha="center", va="center"); save_fig(fig, OUT / "finetune_lr_schedule.png"); return
    h = _load_json(path)
    epochs, _, _, lr = _curve(h)
    if np.all(np.isnan(lr)):
        ax.text(0.5, 0.5, "lr not logged in history", ha="center", va="center",
