...
def main() -> None:
    ...
    h = ax.hexbin(v_T[ok & ~flag], v_perp[ok & ~flag], gridsize=80, mincnt=10,
                   cmap="Greys", bins="log",
                   extent=[0, 400, 0, 200])
    ...
    ax.scatter(v_T[flagged_idx], v_perp[flagged_idx], s=2.5, color="#d62728",
    ...
    for r_iso, color, lab in [(100, "#9467bd", "100 km/s from LSR"),
        ...
        if in_box.any():
            ax.plot(cx[in_box], cy[in_box], color=color, lw=0.9, ls="--", alpha=0.85,
                     label=lab)
    ax.axvline(V_LSR, color="orange", lw=0.8, ls=":",
                label=f"$V_{{\\rm LSR}}$ = {V_LSR:.0f}")
    ax.set_xlim(0, 400); ax.set_ylim(0, 200)
    ax.set_xlabel(r"$v_T$ (km/s)"); ax.set_ylabel(r"$\sqrt{v_R^2+v_z^2}$ (km/s)")
    ...
    ax.legend(fontsize=7, loc="upper right", frameon=True, framealpha=0.95,
              facecolor="white", edgecolor="0.4", markerscale=3)
    ...
    ax.axvline(threshold, color="#d62728", lw=1.4, ls="--",
    ...
    bg_x, bg_y = radec_to_galactic_mollweide(sub.ra_deg.iloc[bg_idx].to_numpy(),
                                             sub.dec_deg.iloc[bg_idx].to_numpy())
    ax.scatter(bg_x, bg_y, s=0.4, alpha=0.10, color="0.5", rasterized=True,
                label=f"in-distribution ({len(sub) - len(f_sub):,})")
    if len(f_sub):
        x, y = radec_to_galactic_mollweide(f_sub.ra_deg.to_numpy(),
                                           f_sub.dec_deg.to_numpy())
        ax.scatter(x, y, s=1.5, alpha=0.65, color="#d62728", rasterized=True,
