...
def _curves_panel(ax):
    ...
    ax.plot(sigma, nll, color=PALETTE["tier3"], lw=2.4, label=r"$\beta = 0$ (plain NLL)")
    ax.plot(
        sigma,
        bnll - bnll.min() + nll.min(),
        color=PALETTE["tier1"],
        lw=2.4,
    ...
    ax.axvline(residual, color=PALETTE["ink"], ls=":", lw=1.6, label=r"$\sigma$ = residual (ideal)")
    ax.set_xlabel(r"predicted $\sigma$  (residual = 1)")
    ax.set_ylabel("loss value")
    ax.set_title("Same residual — where each loss prefers σ to land", color=PALETTE["navy"])
...
def _empirical_panel(ax):
    ...
    bars = ax.bar(
        x, pull_sig, color=PALETTE["navy_light"], edgecolor="white", linewidth=1.4, width=0.65
    )
    ax.axhspan(
        0.8,
        1.2,
    ...
    ax.axhline(1.0, color=PALETTE["tier1"], lw=2.2, ls="--", label="ideal = 1.0")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, fontsize=12)
    ax.set_ylim(0, 1.4)
    ax.set_ylabel(r"empirical pull $\sigma_{\rm MAD}$  (Y16)")
    ax.set_title(r"Loss → calibrated $\sigma$  (Stream-1 held-out)", color=PALETTE["navy"])
...
def main() -> int:
    apply_style()
    fig, axes = plt.subplots(1, 3, figsize=(22, 7.5), gridspec_kw={"width_ratios": [1.0, 1.0, 1.0]})
