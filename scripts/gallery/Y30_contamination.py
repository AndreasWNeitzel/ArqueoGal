...
def main() -> int:
    ...
    ok = (np.isfinite(am_truth) & np.isfinite(am_pred)
          & np.isfinite(mh_truth) & np.isfinite(mh_pred))
    ...
    gs = fig.add_gridspec(1, 3, width_ratios=[0.85, 1.20, 1.0], wspace=0.32,
    ...
    ax.text(0.5, -0.18,
            f"overall purity = {purity*100:.1f}%   "
            f"(α-cut at {ALPHA_DIVIDER:.2f} dex)",
    ...
    ax.hexbin(mh_truth[correct], am_truth[correct], gridsize=70,
              extent=(-1.6, 0.55, -0.10, 0.42),
              mincnt=1, bins="log", cmap="Greys", alpha=0.85)
    ax.scatter(mh_truth[~correct], am_truth[~correct],
               s=4, color=PALETTE["tier3"], alpha=0.45,
               edgecolor="none",
               label=f"contaminated  n={int((~correct).sum()):,}")
    ax.axhline(ALPHA_DIVIDER, color=PALETTE["accent"], lw=1.6, ls="--",
    ...
    ax.set_title("Chemistry plane — contaminated stars in red",
                 color=PALETTE["navy"])
    ...
    for mask, color, lab in cells:
        if int(mask.sum()) == 0:
            continue
        ax.hist(delta[mask], bins=80, range=(-0.20, 0.20),
                histtype="stepfilled", alpha=0.55, color=color,
    ...
    ax.set_title("Residual by truth/pred component",
                 color=PALETTE["navy"])
