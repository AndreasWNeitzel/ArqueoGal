...
def _column(ax, x, n_visible, n_total, color, label):
    ...
    for y in ys:
        c = mpatches.Circle((x, y), 0.16, facecolor=color, edgecolor="white",
                            linewidth=1.2)
        ax.add_patch(c)
    if n_total > n_visible:
        ax.text(x, (y_top + y_bot) / 2, "⋮",
                ha="center", va="center", fontsize=22, color=color)
    ax.text(x, 1.1, label, ha="center", va="top", fontsize=11,
...
def _connect(ax, x0, x1, n0, n1):
    ...
    for a in y0:
        for b in y1:
            ax.plot([x0 + 0.16, x1 - 0.16], [a, b],
                    color=PALETTE["mist"], lw=0.4, alpha=0.55, zorder=0)
...
def main() -> int:
    ...
    labels = ["INPUT\n140-D", "Hidden\n256", "Hidden\n256",
              "Hidden\n256", "OUTPUT\n5 means + 5 σ"]
    colors = [PALETTE["navy_light"], PALETTE["navy"], PALETTE["navy"],
              PALETTE["navy"], PALETTE["accent"]]
    ...
    for y, name in zip(out_y, out_labels):
        ax.text(xs[-1] + 0.55, y, f"{name}  μ, σ",
                ha="left", va="center", fontsize=11.5, color=PALETTE["ink"],
                fontweight="bold")
