...
def _load_norm_coefs(path: Path, n_rows: int = 120_000) -> dict:
    ...
    norm_cols = [f"bp_coef_norm_{k}" for k in range(1, 55) if _have(schema, f"bp_coef_norm_{k}")] \
              + [f"rp_coef_norm_{k}" for k in range(1, 55) if _have(schema, f"rp_coef_norm_{k}")]
    # raw columns for the pre-z-score ratio
    raw_cols = ["bp_coef_0"] + [f"bp_coef_{k}" for k in range(1, 55) if _have(schema, f"bp_coef_{k}")] \
             + ["rp_coef_0"] + [f"rp_coef_{k}" for k in range(1, 55) if _have(schema, f"rp_coef_{k}")]
    ...
    for band in ("bp", "rp"):
        ...
        for k in range(1, 55):
            ...
            if ck is None:
                continue
            ratio = ck.to_numpy(dtype=float) / np.where(np.abs(c0.to_numpy(dtype=float)) > 1e-30,
                                                        c0.to_numpy(dtype=float), np.nan)
            pre[f"{band}_{k}"] = ratio

    post = {col.replace("_coef_norm_", "_").replace("bp_", "bp_").replace("rp_", "rp_"):
            df[col].to_numpy(dtype=float) for col in norm_cols}
    # rename keys consistently: "bp_coef_norm_12" -> "bp_12"
    post = {k.replace("bp_coef_norm_", "bp_").replace("rp_coef_norm_", "rp_"): v
...
def hermite_zscore_per_coef() -> None:
        ...
        for k in range(1, 55):
            ...
            if pre_vals is not None:
                ...
                if len(pv) > 10:
                    ...
                    if hi > lo:
                        ax.hist(pv, bins=np.linspace(lo, hi, 40),
                                color="#9e9e9e", alpha=0.6, density=True, label="pre")
            if post_vals is not None:
                qv = post_vals[np.isfinite(post_vals)]
                if len(qv) > 10:
                    lo, hi = np.percentile(qv, [0.5, 99.5])
                    if hi > lo:
                        ax.hist(qv, bins=np.linspace(lo, hi, 40),
                                color="#d62728", alpha=0.6, density=True, label="post")
    ...
    # Shared legend
    handles = [
        plt.Rectangle((0, 0), 1, 1, color="#9e9e9e", alpha=0.6, label="pre z-score (raw $c_k/c_0$)"),
        plt.Rectangle((0, 0), 1, 1, color="#d62728", alpha=0.6, label="post z-score (emit-time)"),
    ]
    fig.legend(handles=handles, loc="upper center", ncol=2, fontsize=11, bbox_to_anchor=(0.5, 0.995))
    fig.suptitle(
...
def frozen_stats_v1_vs_stream3() -> None:
    ...
    axes[0].plot(idx, s3_mu, "s--", color="#d62728", lw=1.2, ms=3, label="Stream 3 (frozen v1 stats)")
    axes[0].axhline(0, color="k", lw=0.5, ls=":")
    axes[0].set_ylabel(r"empirical mean of $c_k^{\rm norm, z}$")
    axes[0].set_title("Per-coefficient post-z-score empirical mean  —  should be ~0 on both streams "
                      "if frozen-stats contract is honoured",
                      fontsize=11, fontweight="semibold")
    axes[0].legend()

    axes[1].plot(idx, s1_sd, "o-", color="#1f77b4", lw=1.2, ms=3, label="Stream 1 (v1 fit)")
    axes[1].plot(idx, s3_sd, "s--", color="#d62728", lw=1.2, ms=3, label="Stream 3 (frozen v1 stats)")
    ...
    axes[1].set_title("Per-coefficient post-z-score empirical std  —  should be ~1 on Stream 1;"
    ...
    msg = (rf"Stream-1 max $|\mu|$ = {np.nanmax(np.abs(s1_mu)):.3f},  "
    ...
    axes[0].text(0.01, 0.97, msg, transform=axes[0].transAxes, ha="left", va="top",
