...
def _z_hist_panel(ax, df):
    ...
    bands = [
        (am < q33, PALETTE["navy_light"], rf"low α  ([α/M] < {q33:+.2f})"),
        ((am >= q33) & (am < q66), PALETTE["accent"], rf"mid α  ({q33:+.2f} ≤ [α/M] < {q66:+.2f})"),
        (am >= q66, PALETTE["tier3"], rf"high α  ([α/M] ≥ {q66:+.2f})"),
    ]
    ...
    ax.set_title(r"$Z$-distribution, split by [α/M] tertile", color=PALETTE["navy"])
...
def main() -> int:
    ...
    plt.subplots_adjust(wspace=0.32, top=0.84, bottom=0.12, left=0.05, right=0.97)
    _hex(
        axes[0],
        R,
        Z,
