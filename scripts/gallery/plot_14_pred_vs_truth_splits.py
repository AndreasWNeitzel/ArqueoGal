...
def main() -> None:
    ...
    splits = (("Train (LOO kNN)", Y[tri], pred_tr),
    ...
    for r, (name, Yt, Yp) in enumerate(splits):
        for c, (lbl, axlbl, lim) in enumerate(LABELS):
            ...
            ax.hexbin(Yt[m, c], Yp[m, c], gridsize=50, mincnt=5, cmap="viridis",
                       bins="log", extent=[lim[0], lim[1], lim[0], lim[1]])
            ax.plot(lim, lim, color="red", lw=0.6, ls="--")
            ax.set_xlim(lim); ax.set_ylim(lim); ax.set_aspect("equal")
            if r == 2: ax.set_xlabel(f"truth {axlbl}", fontsize=8)
            ax.set_ylabel(f"{name}\npred {axlbl}" if c == 0 else f"pred {axlbl}",
                            fontsize=8)
    ...
    fig.suptitle("Pred vs truth on 70/15/15 splits (kNN+strong-contrastive-v2 hybrid)",
                  fontsize=11)
    fig.tight_layout()
