...
from arqueogal.xp_abundances.main.data import (  # noqa: E402
    FeatureLayout, stratified_split_ids,
)
from arqueogal.xp_abundances.main.ood import (  # noqa: E402
    fit_mahalanobis_ood, score_mahalanobis_ood,
)
...
def _load() -> tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
    ...
    print(f"[H9] full pool: {len(df):,} stars; "
...
def main() -> int:
    ...
    bundle = fit_mahalanobis_ood(
        X108[train_mask], p_threshold=P_THRESHOLD, regularization=1e-6,
    )
    print(f"[H9] fitted bundle: dim={bundle.feature_dim}, "
    ...
    print(f"[H9] held-out OOD-flag rate: {flag_hold.mean():.3%} "
    ...
    gs = fig.add_gridspec(2, 3, hspace=0.40, wspace=0.32,
                          top=0.96, bottom=0.06, left=0.05, right=0.97)
    ...
    ax.hist(np.clip(d_train, 0, x_max), bins=bins, density=True, alpha=0.55,
            color="#1f77b4", label=f"training (n={int(train_mask.sum()):,})")
    ax.hist(np.clip(d_hold, 0, x_max), bins=bins, density=True, alpha=0.55,
            color="#ff7f0e", label=f"held-out (n={int(holdout_mask.sum()):,})")
    ax.axvline(bundle.threshold, color="#d62728", lw=1.4,
               label=f"τ = {bundle.threshold:.2f} (train p99)")
    ...
    ax.plot([0, ymax], [0, ymax], color="#d62728", lw=1.0, ls="--",
            label=r"$y=x$ (perfect $\chi^2$)")
    ax.axhline(bundle.threshold ** 2, color="#7f7f7f", lw=0.8, ls=":",
    ...
    ax.set_title(r"Q-Q: training $d^2$ vs $\chi^2_{108}$  "
    ...
    def _pc_panel(ax, ix, iy):
        ...
        sc = ax.scatter(
            xs[order], ys[order], c=ds[order],
            cmap="viridis", s=3.0, alpha=0.55,
            vmin=0, vmax=min(np.nanpercentile(d_hold, 99.5), bundle.threshold * 2.0),
            edgecolors="none",
        )
        ...
        ax.scatter(
            xs[red_idx], ys[red_idx], facecolors="none",
            edgecolors="#d62728", linewidths=0.45, s=14, alpha=0.85,
        )
        ...
        e = Ellipse(
            ...
            facecolor="none", edgecolor="#d62728", lw=1.6, ls="--",
            label=rf"$d = \tau$ (other PCs = $\mu$)",
        )
        ax.add_patch(e)
        ...
        ax.set_xlim(-3.0 * bundle.threshold * np.sqrt(eigvals[ix]),
                    3.0 * bundle.threshold * np.sqrt(eigvals[ix]))
        ax.set_ylim(-3.0 * bundle.threshold * np.sqrt(eigvals[iy]),
    ...
    label_truth_cols = ("teff_apogee", "logg_apogee", "mh_apogee",
    ...
    label_bundle = fit_mahalanobis_ood(
        Y_train_truth, p_threshold=P_THRESHOLD, regularization=1e-8,
    )
    print(f"[H9] label bundle: dim={label_bundle.feature_dim}, "
          f"τ_label = {label_bundle.threshold:.3f}")

    pred_cols = ("teff_pred", "logg_pred", "mh_pred",
    ...
    ax.hist(np.clip(d_label_hold, 0, x_max_label), bins=bins_label,
            density=True, alpha=0.65, color="#ff7f0e",
            label=f"held-out predictions (n={int(holdout_mask.sum()):,})")
    ax.axvline(label_bundle.threshold, color="#d62728", lw=1.4,
    ...
    for name, d, color in (("training (pred labels)", d_train_pred, "#1f77b4"),
                            ("held-out (pred labels)", d_label_hold, "#ff7f0e")):
        ds = np.sort(d[np.isfinite(d)])
        cdf = np.arange(1, len(ds) + 1) / len(ds)
        ax.plot(ds, cdf, color=color, lw=1.5, label=name)
    ax.axvline(label_bundle.threshold, color="#d62728", lw=1.2,
    ...
    for t in (1, 2, 3):
        ...
        if m.any():
            ax.scatter(
                d_hold[m], d_label_hold[m], s=sizes[t],
                c=palette[t], alpha=alphas[t],
    ...
    ax.axvline(bundle.threshold, color="#d62728", lw=1.6, ls="--",
               label=rf"T3 gate $\tau_{{\rm XP}}$ = {bundle.threshold:.2f}")
    ax.axvspan(bundle.threshold, 1e6, color="#d62728", alpha=0.07,
               zorder=0)
    # T2 boundary (above τ_label) — horizontal orange.
    ax.axhline(label_bundle.threshold, color="#ff7f0e", lw=1.6, ls="--",
               label=rf"T2 gate $\tau_{{\rm label}}$ = {label_bundle.threshold:.2f}")
    ax.axhspan(label_bundle.threshold, 1e6, color="#ff7f0e", alpha=0.10,
               zorder=0)
    ax.set_xlabel("XP-Mahalanobis $d_{\\rm XP}$ (108-D)")
    ax.set_ylabel("label-Mahalanobis $d_{\\rm label}$ (5-D)")
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlim(max(1.0, np.nanmin(d_hold) * 0.9),
                np.nanpercentile(d_hold, 99.95))
    ax.set_ylim(max(0.05, np.nanmin(d_label_hold) * 0.9),
                np.nanpercentile(d_label_hold, 99.95))
