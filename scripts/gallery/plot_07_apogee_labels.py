...
def label_pairwise_hexbin() -> None:
    ...
    pairs = [
        ("mh_apogee",   "alpha_m_apogee", r"$[{\rm M}/{\rm H}]$", r"$[\alpha/{\rm M}]$",
         (-2.2, 0.6), (-0.25, 0.55)),
        ("mh_apogee",   "fe_h_apogee",    r"$[{\rm M}/{\rm H}]$", r"$[{\rm Fe}/{\rm H}]$",
         (-2.2, 0.6), (-2.2, 0.6)),
        ("alpha_m_apogee", "mg_h_apogee", r"$[\alpha/{\rm M}]$",   r"$[{\rm Mg}/{\rm H}]$",
         (-0.25, 0.55), (-1.8, 0.6)),
    ]
    ...
    for ax, (x, y, xl, yl, xlim, ylim) in zip(axes.flat, pairs):
        ...
        hb = ax.hexbin(xv[m], yv[m], gridsize=80, cmap="magma", bins="log",
    ...
    fig.suptitle("APOGEE DR19 label pairwise hexbin  —  Mészáros+2025-corrected, Stream 1",
...
def label_nan_rates() -> None:
    ...
    ax.set_title(rf"APOGEE DR19 per-label NaN rate  —  n={n:,} Stream 1 stars  "
                 r"(drives `mask=` path in beta_nll_block_cholesky)",
                 fontsize=11, fontweight="semibold")
    # annotations
    for b, pct in zip(bars, fracs):
        ax.text(b.get_width() + 0.15, b.get_y() + b.get_height() / 2, f"{pct:.2f}%",
