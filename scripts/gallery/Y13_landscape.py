...
def _panel_a(ax):
    ...
    bars = ax.barh(y, n_stars, color=colors, edgecolor="white", linewidth=1.4, height=0.65)
    ax.set_yticks(y)
    ...
    ax.set_title("Catalog size", color=PALETTE["navy"], fontsize=15)
...
def _panel_b(ax):
    names = [c[0] for c in CATALOGS]
    flags = np.array([[c[3], c[4], c[5]] for c in CATALOGS], dtype=bool)
    ...
    ax.set_yticklabels(names, fontsize=11)
    ax.invert_yaxis()
    ax.set_title("Methodological scoreboard", color=PALETTE["navy"], fontsize=15)
    ax.grid(False)
    ...
    for i in range(n):
        ...
        for j in range(len(label_axis)):
            ...
            sym = "✓" if ok else "✗"
            ax.scatter(j, i, s=460, c=color, edgecolor="white", linewidth=1.6, zorder=2)
            ax.text(
                j,
                i,
                sym,
...
def main() -> int:
    apply_style()
    fig, axes = plt.subplots(1, 2, figsize=(20, 8), gridspec_kw={"width_ratios": [1.0, 1.0]})
