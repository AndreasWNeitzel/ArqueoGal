...
def _schematic_panel(ax):
    ...
    # Background scatter.
    ax.scatter(mh, am, s=18, c=weight, cmap="viridis",
               vmin=0, vmax=1, edgecolor="white", linewidth=0.4, zorder=2)
    # Concentric kernel rings at 1σ and 2σ.
    for r, ls, lab in [(sigma, "-", r"$1\sigma$"),
                       (2 * sigma, "--", r"$2\sigma$")]:
        ring = mpatches.Circle(anchor, r, fill=False, lw=1.6, ls=ls,
                               edgecolor=PALETTE["accent"], zorder=3)
        ax.add_patch(ring)
        ax.text(anchor[0] + r, anchor[1] + 0.005, lab,
                color=PALETTE["accent"], fontsize=11, fontweight="bold")
    # Anchor marker.
    ax.scatter(*anchor, marker="*", s=520, color="white",
               edgecolor=PALETTE["accent"], linewidth=2.4, zorder=4,
               label="anchor star")
    ax.set_xlabel("[M/H]  (label space)")
    ax.set_ylabel(r"[$\alpha$/M]")
    ax.set_xlim(-2.0, 0.6)
    ax.set_ylim(-0.10, 0.42)
    ax.set_title("Anchor + Gaussian-kernel soft positives",
                 color=PALETTE["navy"])
...
def _formula_panel(ax):
    ...
    ax.add_patch(rect)
    ax.text(0.5, 0.92, "SupCon-soft-positive loss",
    ...
    ax.text(0.5, 0.45,
            ...
            ha="center", va="center", fontsize=13, color=PALETTE["ash"],
            fontstyle="italic")
    ax.text(0.5, 0.34,
