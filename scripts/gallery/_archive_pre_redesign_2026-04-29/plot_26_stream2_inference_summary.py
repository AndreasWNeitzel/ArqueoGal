...
def main() -> None:
    ...
    gs = fig.add_gridspec(2, 3, hspace=0.35, wspace=0.30, width_ratios=[1.05, 1, 1])
    ...
    for i, _e in enumerate(elements):
        pct1 = 100 * t1[i] / n
        ax.text(
            i,
            t1[i] * 1.02,
    ...
    h = ax.hexbin(
        ...
        cmap="viridis",
        bins="log",
        extent=[3500, 6500, 3500, 6500],
    )
    ax.plot([3500, 6500], [3500, 6500], "r--", lw=1, alpha=0.7, label="1-to-1")
    ...
    ax.hist(score, bins=bins, color="#1f77b4", alpha=0.65, label=f"all ({len(score):,})")
    flagged = df.loc[df.ood_joint_flag.astype(bool), "ood_mahalanobis_score"].dropna()
    ax.hist(
        flagged, bins=bins, color="#d62728", alpha=0.75, label=f"OOD-flagged ({len(flagged):,})"
    )
