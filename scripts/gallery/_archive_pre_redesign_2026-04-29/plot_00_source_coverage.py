...
def main() -> None:
    ...
    s1 = pd.read_parquet(
        S1, columns=["source_id", "ra_deg", "dec_deg", "g_mag", "teff_apogee", "logg_apogee"]
    )
    s3 = pd.read_parquet(
        S3, columns=["source_id", "ra_deg", "dec_deg", "g_mag", "teff_andrae", "logg_andrae"]
    )
    ...
    if S2_FEAT.exists():
        ...
        s2 = s2.rename(columns={"ra": "ra_deg", "dec": "dec_deg", "phot_g_mean_mag_corr": "g_mag"})
    else:
        s2 = None
    ...
    if s2 is not None:
        ...
        raw_s2_teff = np.array([])
        raw_s2_logg = np.array([])
    ...
    for col, (df, name, color) in enumerate(streams_sky):
        ...
        if df is None:
            ax.text(
                0,
                0,
                "Stream 2 features not yet built",
                ha="center",
                va="center",
                fontsize=10,
    ...
    ax_g.axvline(17.0, color="red", lw=0.9, ls="--", label="G = 17 (Pipeline-1 cap)")
    ...
    if s2 is not None:
        _kiel_panel(
            ...
            raw_s2_logg,
            raw_s2_teff,
            raw_s2_logg,  # S2 not yet ML-cut; show same data
