...
def _knn_pred(model, X, tri, vai, tei, Y):
    ...
    Y_pool = Y[tri]
    return (
        np.median(Y_pool[idx_tr], axis=1),
...
def main() -> None:
    ...
    splits = (("Train (LOO)", Y[tri], pred_tr), ("Val", Y[vai], pred_va), ("Test", Y[tei], pred_te))
        ...
        for col, (X_plot, Y_plot, labels, title) in enumerate(
            ...
            for k in range(3):
                ...
                ax.scatter(
                    ...
                    alpha=0.4,
                    rasterized=True,
                    label=COMP_LABELS[k] if (r, col) == (0, 0) else None,
                )
            for k in range(3):
                ax.scatter(
                    mt[k, 0],
