...
def _kiel(ax, x, y, title, anno=None):
    ...
    ax.set_xlim(KIEL_X[1], KIEL_X[0])
    ax.set_ylim(KIEL_Y[1], KIEL_Y[0])
...
def main() -> None:
    ...
    Y_pool = Y[tri]
    splits = (
        ("Train (LOO kNN)", Y[tri], np.median(Y_pool[idx_tr], axis=1)),
    ...
    for r, (name, Yt, Yp) in enumerate(splits):
        ...
        _kiel(axes[r, 0], teff_t, logg_t, title=("Kiel: truth" if r == 0 else ""), anno=f"n={n:,}")
        ...
        _kiel(
            ...
            anno=(
                f"Teff: RMSE={rmse_t:.0f} K, bias={bias_t:+.0f}\n"
                r"$\log g$: " + f"RMSE={rmse_g:.3f}, bias={bias_g:+.3f}"
            ),
        )
        _chem(axes[r, 2], mh_t, am_t, title=("Chemistry: truth" if r == 0 else ""))
        ...
        _chem(
            ...
            anno=(
                f"[M/H]: RMSE={rmse_m:.3f}, bias={bias_m:+.3f}\n"
                r"[$\alpha$/M]: " + f"RMSE={rmse_a:.3f}, bias={bias_a:+.3f}"
            ),
        )

    fig.suptitle(
        "Kiel and chemistry truth-vs-pred on 70/15/15 splits (immediate post-training)", fontsize=11
    )
    fig.tight_layout()
