...
def _load_zscored(
    ...
    rp_c0_key = (
        "rp_c0_z"
        if "rp_c0_z" in schema.columns
        else ("rp_c0_log" if "rp_c0_log" in schema.columns else None)
    )
    if bp_c0_key:
        c0_cols.append(bp_c0_key)
    if rp_c0_key:
        c0_cols.append(rp_c0_key)
...
def main() -> None:
    ...
    if PROV.exists():
        try:
            ...
            fingerprint = (
                payload.get("extra", {}).get("frozen_stats_basis_fingerprint_sha256")
    ...
    idx = np.arange(1, 55)
    for axis_idx, (band, m1, m2, m3) in enumerate([("BP", bp1, bp2, bp3), ("RP", rp1, rp2, rp3)]):
        ax = axes[axis_idx]
        q1 = np.nanpercentile(m1, [16, 50, 84], axis=0)
        ax.fill_between(
            idx, q1[0], q1[2], color="#1f77b4", alpha=0.30, label=f"Stream 1 (n={len(m1):,})"
        )
        ax.plot(idx, q1[1], color="#1f77b4", lw=0.9)
        if m2 is not None:
            q2 = np.nanpercentile(m2, [16, 50, 84], axis=0)
            ax.fill_between(
                idx, q2[0], q2[2], color="#9467bd", alpha=0.25, label=f"Stream 2 (n={len(m2):,})"
            )
            ax.plot(idx, q2[1], color="#9467bd", lw=0.9, ls="-.")
        q3 = np.nanpercentile(m3, [16, 50, 84], axis=0)
        ax.fill_between(
            idx, q3[0], q3[2], color="#d62728", alpha=0.30, label=f"Stream 3 (n={len(m3):,})"
        )
    ...
    # c0 overlay
    ax = axes[2]
    for c0, color, name in [
        (bpc01, "#1f77b4", "S1 (z-scored)"),
        ...
        if m.sum() < 100:
            continue
        bins = np.linspace(np.nanpercentile(c0, 0.5), np.nanpercentile(c0, 99.5), 61)
        ax.hist(
            c0[m],
            bins=bins,
            color=color,
