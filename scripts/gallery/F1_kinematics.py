...
def main() -> int:
    ...
    plt.subplots_adjust(top=0.92, hspace=0.32, wspace=0.30, left=0.05, right=0.97, bottom=0.07)
    ...
    if have_pm:
        ...
        ax.set_ylim(-30, 30)
        ax.set_aspect("equal")
        plt.colorbar(hb, ax=ax, label=r"$\log_{10}$ N", fraction=0.046, pad=0.04)
    else:
        ax.text(
            0.5,
            0.5,
            "pmra/pmdec unavailable",
    ...
    # (b) Toomre diagram.
    ax = axes[0, 1]
    v_perp = np.sqrt(kin["v_R_kms"].to_numpy() ** 2 + kin["v_z_kms"].to_numpy() ** 2)
    ...
    hb = ax.hexbin(
        lz[ok], jr[ok], gridsize=80, extent=(-3, 3.5, 0, 8), mincnt=1, bins="log", cmap="viridis"
    )
    plt.colorbar(hb, ax=ax, label=r"$\log_{10}$ N", fraction=0.046, pad=0.04)
    ax.set_xlim(-3, 3.5)
    ax.set_ylim(0, 8)
    ...
    hb = ax.hexbin(
        lz[ok], jz[ok], gridsize=80, extent=(-3, 3.5, 0, 5), mincnt=1, bins="log", cmap="viridis"
    )
    plt.colorbar(hb, ax=ax, label=r"$\log_{10}$ N", fraction=0.046, pad=0.04)
    ax.set_xlim(-3, 3.5)
    ax.set_ylim(0, 5)
    ...
    ax.hist(
        ecc, bins=80, range=(0, 1), color="#1f77b4", edgecolor="white", linewidth=0.4, alpha=0.85
    )
    med = float(np.nanmedian(ecc))
    ax.axvline(med, color="#e07b00", lw=2.0, ls="--", label=f"median ecc = {med:.3f}")
    ax.set_xlim(0, 1)
    ...
    fig.suptitle(
        f"F1. Stream-3 kinematics (galpy McMillan+2017 + Staeckel fudge; n = {n:,})",
        fontsize=12,
        fontweight="semibold",
        y=0.985,
    )
