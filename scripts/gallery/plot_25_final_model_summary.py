...
EL_LBL = {"teff": "Teff", "logg": "log g", "mh": "[M/H]", "alpha_m": "[α/M]",
          "mg_h": "[Mg/H]"}
...
def _stress_status() -> dict[str, str]:
    ...
    return {m.group(1): m.group(2)
            for m in re.finditer(r"::(\S+)\s+(PASSED|FAILED)", text)}
...
def main() -> None:
    ...
    if HYBRID.exists():
        ...
        ax.text(0.5, 0.5, "see stage 14:\npred-vs-truth scatter", ha="center",
    ...
    streams = []
    if HYBRID.exists():
        streams.append(("Stream 3", "#d62728",
                          pd.read_parquet(HYBRID, columns=["release_tier"])))
    if HYBRID_S2.exists():
        streams.append(("Stream 2", "#9467bd",
                          pd.read_parquet(HYBRID_S2, columns=["release_tier"])))
    if streams:
        tiers = [1, 2, 3]
        n_s = len(streams)
        width = 0.8 / max(n_s, 1)
        ...
        for i, (name, color, df_t) in enumerate(streams):
            ...
            ax.bar(x + offset, cnts, width=width * 0.95, color=color,
                    label=f"{name} (n={len(df_t):,})")
    ...
    if HYBRID.exists():
        df_h = pd.read_parquet(HYBRID,
                                columns=[f"{e}_hybrid_source" for e in ELEMENTS])
        n = len(df_h)
        bottoms = np.zeros(len(ELEMENTS))
        for src, color in [("regressor", "#1f77b4"), ("knn", "#ff7f0e"),
                            ("regressor_caveat", "#d62728")]:
            fr = np.array([(df_h[f"{e}_hybrid_source"] == src).sum() / n * 100
                            for e in ELEMENTS])
            ax.bar(range(len(ELEMENTS)), fr, bottom=bottoms, color=color,
                    label=src, edgecolor="white", lw=0.4)
        ...
        ax.set_ylabel("% of stars"); ax.set_ylim(0, 100)
    ...
    if HYBRID.exists():
        df_s = pd.read_parquet(HYBRID,
                                columns=[f"prediction_sigma_inflated__{e}" for e in ELEMENTS])
    ...
    if CONT_J.exists():
        ...
        macro = test["macro_f1"]
        bars = ax.bar(["G1", "G2", "G3", "macro"], f1 + [macro],
    ...
    for i, (tid, label) in enumerate(tests):
        ...
        if i >= 4: continue  # only 4 panels in bottom row of gridspec
        ...
        ax.add_patch(patches.Rectangle((0.05, 0.10), 0.9, 0.8,
                                          facecolor=color, edgecolor=edge, lw=1.4))
        ax.text(0.5, 0.7, label, ha="center", va="center", fontsize=10,
                 fontweight="semibold")
        ax.text(0.5, 0.30, outcome, ha="center", va="center", fontsize=10,
