...
def _load(path: Path) -> pd.DataFrame | None:
    ...
    cols = list(set(FLAG_COLS) & set(full)) + [
        "ra_deg",
        "dec_deg",
        "g_mag",
...
def main() -> None:
    ...
    flags = [
        c
        for c in FLAG_COLS
        if (s3 is not None and c in s3.columns) or (s2 is not None and c in s2.columns)
    ]
    rates3 = [
        100.0 * float(s3[f].mean()) if s3 is not None and f in s3.columns else 0.0 for f in flags
    ]
    rates2 = [
        100.0 * float(s2[f].mean()) if s2 is not None and f in s2.columns else 0.0 for f in flags
    ]
    ...
    ax.set_yticks(y)
    label_strs = [f"{f} (active)" if f in ACTIVE_GATES else f"{f} (diag.)" for f in flags]
    ...
    ax.set_title(
        "Per-gate firing rates (S2 vs S3)\n"
    ...
    for df, name, color in [(s3, "Stream 3", "#d62728"), (s2, "Stream 2", "#9467bd")]:
    ...
    for col, df, name in [(0, s3, "Stream 3"), (1, s2, "Stream 2")]:
        ...
        ra = df.ra_deg.iloc[idx].to_numpy()
        dec = df.dec_deg.iloc[idx].to_numpy()
    ...
    for i, (df, name, color) in enumerate(
        [(s3, "Stream 3", "#d62728"), (s2, "Stream 2", "#9467bd")]
    ):
        if df is None or "release_tier" not in df.columns:
            continue
        n = len(df)
        ...
        bars = ax.bar(x + offset, cnts, width=width * 0.95, color=color, label=f"{name} (n={n:,})")
