...
def _stream3_df():
    ...
    return pq.read_table(
        DATA_PROCESSED / "pipeline1_predictions_stream3_v11.parquet", columns=have
    ).to_pandas()
...
def aleatoric_vs_epistemic_per_label() -> None:
    ...
    fig.suptitle(
        "Aleatoric vs epistemic uncertainty per label  —  "
...
def ood_mahalanobis_distribution() -> None:
    ...
    if "ood_mahalanobis_score" not in df.columns:
        ax.text(0.5, 0.5, "ood_mahalanobis_score column absent", ha="center", va="center")
    ...
    ax.hist(np.clip(vals, lo, hi), bins=70, color="#1f77b4", edgecolor="#333", alpha=0.85)
    # 99th percentile threshold from training (we don't have that explicitly here,
    # so use the Stream-3 p99 as a proxy marker)
    p99 = np.percentile(vals, 99)
    ax.axvline(p99, color="#d62728", lw=1.4, ls="--", label=f"Stream-3 p99 = {p99:.2f}")
    ax.set_yscale("log")
    ax.set_xlabel("Mahalanobis score  (108-D XP block)")
    ax.set_ylabel("count  (log)")
    ax.set_title(
        f"Stream 3  —  Mahalanobis distribution  (n={len(vals):,})",
        fontsize=11,
        fontweight="semibold",
    )
    ax.legend()
...
def ood_disagreement_distribution() -> None:
    ...
    try:
        ...
        ax.hist(np.clip(ratios, 0, 2), bins=60, color="#9467bd", edgecolor="#333", alpha=0.85)
        ax.axvline(0.5, color="#d62728", lw=1.4, ls="--", label=r"disagreement threshold 0.5")
        ax.set_xlabel(r"$\langle\sigma_{\rm epi} / \sigma_{\rm total}\rangle_{\rm labels}$")
        ax.set_ylabel("count")
        ax.set_yscale("log")
        ax.set_title(
            f"Stream 3 ensemble disagreement  (n={len(ratios):,})",
            fontsize=11,
            fontweight="semibold",
        )
        ax.legend()
...
def ood_joint_decision_plot() -> None:
    ...
    hb = ax.hexbin(
        ...
        bins="log",
        mincnt=1,
        extent=(0, min(50, np.nanpercentile(maha, 99.5)), 0, 2),
    )
    plt.colorbar(hb, ax=ax, shrink=0.85, pad=0.02, label="log N")
    ax.axvline(mh_thr, color="#1f77b4", lw=1.2, ls="--", label=f"maha p99 = {mh_thr:.2f}")
    ax.axhline(da_thr, color="#2ca02c", lw=1.2, ls="--", label=f"disagreement = {da_thr}")
    # shade joint-flag region
    ax.fill_between(
        [mh_thr, 60], da_thr, 2.5, color="#d62728", alpha=0.2, label="joint-flag (both fire)"
    )
...
def regime_b_envelope_footprint() -> None:
    ...
    df = pq.read_table(
        ...
        columns=["teff_apogee", "logg_apogee", "b_deg"],
    ).to_pandas()
    ...
    fig, ax = plt.subplots(figsize=(9, 7))
    ax.hexbin(
        df.loc[m, "teff_apogee"],
        df.loc[m, "logg_apogee"],
        gridsize=70,
        cmap="Greys",
        bins="log",
        mincnt=1,
    ...
    ax.hexbin(
        df.loc[low_b, "teff_apogee"],
        df.loc[low_b, "logg_apogee"],
        gridsize=70,
        cmap="Reds",
        bins="log",
        mincnt=5,
    ...
    env = Polygon(
        [(4750, 2.1), (5500, 2.1), (5500, 0.5), (4750, 0.5)],
        closed=True,
        edgecolor="#d62728",
        facecolor="none",
        lw=2,
    ...
    ax.set_title(
        r"Regime B exclusion envelope on Stream 1 Kiel diagram  "
        r"(red = $|b|<5°$ subsample)",
        fontsize=11,
        fontweight="semibold",
    )
