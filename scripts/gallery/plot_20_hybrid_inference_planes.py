...
def _density_panel(ax, x, y, *, x_lo, x_hi, y_lo, y_hi, nx, ny,
                    xlab, ylab, title, n):
    ...
    if m.sum() < 50:
        ax.text(0.5, 0.5, f"too few finite predictions\nn={int(m.sum())}",
...
def _kiel_panel(ax, teff, logg, *, title, n):
    ...
    _density_panel(ax, teff, logg,
                    x_lo=KIEL_X[0], x_hi=KIEL_X[1],
                    y_lo=KIEL_Y[0], y_hi=KIEL_Y[1],
                    nx=240, ny=140,
                    xlab="Teff (K)", ylab=r"$\log g$ (dex)",
                    title=title, n=n)
...
def _chem_panel(ax, mh, am, *, title, n):
    ...
    _density_panel(ax, mh, am,
                    x_lo=CHEM_X[0], x_hi=CHEM_X[1],
                    y_lo=CHEM_Y[0], y_hi=CHEM_Y[1],
                    nx=240, ny=50,
                    xlab="[M/H] (dex)", ylab=r"[$\alpha$/M] (dex)",
                    title=title, n=n)
...
def _render_for(parquet: Path, stream_label: str, kiel_out: Path, chem_out: Path) -> None:
    ...
    df = pd.read_parquet(
        parquet,
        columns=["teff_hybrid_pred", "logg_hybrid_pred", "mh_hybrid_pred",
                  "alpha_m_hybrid_pred", "teff_hybrid_source", "mh_hybrid_source",
                  "release_tier"],
    )
...
def main() -> None:
    apply_style()
    _render_for(PARQUET_S3, "Stream 3",
                 OUT / "hybrid_kiel.png", OUT / "hybrid_chemistry.png")
    _render_for(PARQUET_S2, "Stream 2",
                 OUT / "hybrid_kiel_stream2.png",
                 OUT / "hybrid_chemistry_stream2.png")


def _render_one_stream(df: pd.DataFrame, stream_label: str,
                        kiel_out: Path, chem_out: Path) -> None:
    ...
    _kiel_panel(axes[0], t12["teff_hybrid_pred"].to_numpy(),
                 t12["logg_hybrid_pred"].to_numpy(),
                 title="Tier 1 + Tier 2 UNION\nthe full trustworthy catalog",
                 n=len(t12))
    _kiel_panel(axes[1], t1["teff_hybrid_pred"].to_numpy(),
                 t1["logg_hybrid_pred"].to_numpy(),
                 title="Tier 1 (per-star science)\nall regressor", n=len(t1))
    _kiel_panel(axes[2], t2_reg["teff_hybrid_pred"].to_numpy(),
                 t2_reg["logg_hybrid_pred"].to_numpy(),
                 title="Tier 2 caveat\nteff = regressor", n=len(t2))
    _kiel_panel(axes[3], t2_knn["teff_hybrid_pred"].to_numpy(),
                 t2_knn["logg_hybrid_pred"].to_numpy(),
                 title="Tier 2 rescued\nteff = kNN-median", n=len(t2))
    _kiel_panel(axes[4], t3["teff_hybrid_pred"].to_numpy(),
                 t3["logg_hybrid_pred"].to_numpy(),
                 title="Tier 3 DO NOT USE\nany OOD or NaN", n=len(t3))
    ...
    fig, axes = plt.subplots(1, 5, figsize=(20, 4.4))
    _chem_panel(axes[0], t12["mh_hybrid_pred"].to_numpy(),
                 t12["alpha_m_hybrid_pred"].to_numpy(),
                 title="Tier 1 + Tier 2 UNION\nthe full trustworthy catalog",
                 n=len(t12))
    _chem_panel(axes[1], t1["mh_hybrid_pred"].to_numpy(),
                 t1["alpha_m_hybrid_pred"].to_numpy(),
                 title="Tier 1 (per-star science)\nall regressor", n=len(t1))
    _chem_panel(axes[2], t2_reg_mh["mh_hybrid_pred"].to_numpy(),
                 t2_reg_mh["alpha_m_hybrid_pred"].to_numpy(),
                 title="Tier 2 caveat\n[M/H] = regressor", n=len(t2))
    _chem_panel(axes[3], t2_knn_mh["mh_hybrid_pred"].to_numpy(),
                 t2_knn_mh["alpha_m_hybrid_pred"].to_numpy(),
                 title="Tier 2 rescued\n[M/H] = kNN-median", n=len(t2))
    _chem_panel(axes[4], t3["mh_hybrid_pred"].to_numpy(),
