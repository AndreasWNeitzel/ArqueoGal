...
def main() -> int:
    ...
    tess = pd.read_parquet(
        TESS_GAIA,
        columns=["source_id", "numax_muhz", "e_numax_muhz"],
    ).drop_duplicates("source_id")
    ...
    hb = ax.hexbin(
        log_g_seis, log_g_xp, gridsize=70, extent=extent, mincnt=1, bins="log", cmap="viridis"
    )
    ax.plot(
        [extent[0], extent[1]],
        [extent[0], extent[1]],
    ...
    ax.set_title(r"$\log g$ from XP vs $\log g$ from seismology", color=PALETTE["navy"])
    ...
    ax.text(
        ...
        fontweight="bold",
        color=PALETTE["ink"],
        bbox=dict(boxstyle="round,pad=0.35", facecolor="white", edgecolor=PALETTE["mist"]),
    )
    ...
    ax.hist(
        ...
        linewidth=0.4,
        density=True,
        label=r"$\Delta = \log g_{\rm XP} - \log g_{\rm seis}$",
    )
    xs = np.linspace(-0.5, 0.5, 400)
    gauss = (1.0 / (rsig * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((xs - bias) / rsig) ** 2)
    ax.plot(xs, gauss, color=PALETTE["accent"], lw=2.4, label=r"$N(\rm bias, \sigma_{\rm MAD})$")
    ax.axvline(0.0, color=PALETTE["ink"], lw=1.0, alpha=0.5)
    ax.set_xlim(-0.5, 0.5)
    ax.set_xlabel(r"$\Delta \log g$ (dex)")
    ax.set_ylabel("density")
    ax.set_title("Residual against asteroseismic truth", color=PALETTE["navy"])
    verdict = PALETTE["tier1"] if 0.85 <= pull_std <= 1.20 else PALETTE["tier2"]
    ax.text(
        0.97,
        0.97,
        ...
        fontweight="bold",
        color="white",
        bbox=dict(boxstyle="round,pad=0.35", facecolor=verdict, edgecolor="none"),
    )
