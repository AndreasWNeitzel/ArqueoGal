...
def _load(path: Path) -> pd.DataFrame | None:
    ...
    base_cols = ["ra_deg", "dec_deg", "g_mag", "j_mag", "h_mag", "k_mag",
                 "w1_mag", "w2_mag"]
...
def main() -> None:
    ...
    for i, (name, path, color) in enumerate(STREAMS):
        ...
        x_ok, y_ok = radec_to_galactic_mollweide(
            sub.loc[has_ir, "ra_deg"].to_numpy(),
            sub.loc[has_ir, "dec_deg"].to_numpy())
        x_no, y_no = radec_to_galactic_mollweide(
            sub.loc[~has_ir, "ra_deg"].to_numpy(),
            sub.loc[~has_ir, "dec_deg"].to_numpy())
        ax.scatter(x_ok, y_ok, s=0.4, alpha=0.35, color="#2ca02c", rasterized=True,
                    label=f"IR ({int(has_ir.sum()):,})")
        if (~has_ir).sum():
            ax.scatter(x_no, y_no, s=0.5, alpha=0.6, color="#d62728",
                        rasterized=True,
    ...
    for name, color, df in loaded:
        ...
        ax.plot(centres, rate_pct, "o-", color=color, lw=1.2, ms=4,
                 label=f"{name}")
    ax.set_xlabel("G (mag)")
    ...
    for name, color, df in loaded:
        ...
        ax.scatter((g - k)[idx], (j - k)[idx], s=0.6, alpha=0.20, color=color,
                    rasterized=True, label=f"{name} ({m.sum():,})")
    ax.set_xlabel("G − K (mag)")
    ax.set_ylabel("J − K (mag)")
    ax.set_xlim(0, 5); ax.set_ylim(0, 1.5)
    ax.set_title("Colour-colour overlay")
    ax.legend(fontsize=7, loc="upper left", frameon=True, framealpha=0.95,
              facecolor="white", edgecolor="0.4", markerscale=6)
    ...
    for i, (name, color, df) in enumerate(loaded):
        ...
        ax.bar(x + offset, meds, width=width * 0.95, color=color,
                label=name)
    ...
    fig.suptitle("Stage 06 — IR photometry (2MASS + AllWISE) across S1 / S2 / S3",
                  fontsize=10)
