...
def _kiel_density(ax, df_combined, sid):
    ...
    ax.set_xlim(6500, 3500); ax.set_ylim(5.0, 0.0)
    ax.set_xlabel(r"$T_{\rm eff,\,pred}$ (K)")
    ax.set_ylabel(r"$\log g_{\rm pred}$ (dex)")
    ax.set_title(f"Stream {sid} — Kiel density (T1+T2)",
                 color="#15355f")
    ax.grid(True, alpha=0.20)


def _kiel_scatter(ax, df_t1, df_t2, df_t3, sid):
    ax.scatter(df_t1["teff_pred"], df_t1["logg_pred"],
                s=1.0, alpha=0.18, color=T1_COLOR, edgecolors="none",
                rasterized=True, label=f"T1  n={len(df_t1):,}")
    ax.scatter(df_t2["teff_pred"], df_t2["logg_pred"],
                s=2.5, alpha=0.55, color=T2_COLOR, edgecolors="none",
                rasterized=True, label=f"T2  n={len(df_t2):,}")
    ax.scatter(df_t3["teff_pred"], df_t3["logg_pred"],
                s=2.5, alpha=0.45, color=T3_COLOR, edgecolors="none",
                rasterized=True, label=f"T3  n={len(df_t3):,}")
    ax.set_xlim(6500, 3500); ax.set_ylim(5.0, 0.0)
    ax.set_xlabel(r"$T_{\rm eff,\,pred}$ (K)")
    ax.set_ylabel(r"$\log g_{\rm pred}$ (dex)")
...
def _chem_density(ax, df_combined, sid):
    ...
    ax.set_xlim(-1.6, 0.55); ax.set_ylim(-0.10, 0.42)
    ax.set_xlabel("[M/H] pred (dex)")
    ax.set_ylabel(r"[$\alpha$/M] pred (dex)")
    ax.set_title(f"Stream {sid} — chemistry density (T1+T2)",
                 color="#15355f")
    ax.grid(True, alpha=0.20)


def _chem_scatter(ax, df_t1, df_t2, df_t3, sid):
    ax.scatter(df_t1["mh_pred"], df_t1["alpha_m_pred"],
                s=1.0, alpha=0.18, color=T1_COLOR, edgecolors="none",
                rasterized=True, label=f"T1  n={len(df_t1):,}")
    ax.scatter(df_t2["mh_pred"], df_t2["alpha_m_pred"],
                s=2.5, alpha=0.55, color=T2_COLOR, edgecolors="none",
                rasterized=True, label=f"T2  n={len(df_t2):,}")
    ax.scatter(df_t3["mh_pred"], df_t3["alpha_m_pred"],
                s=2.5, alpha=0.45, color=T3_COLOR, edgecolors="none",
                rasterized=True, label=f"T3  n={len(df_t3):,}")
...
def _sky_panel(ax, df_t1, df_t2, df_t3, sid):
    for df, color, label, s, alpha in [
        ...
        if not len(df):
            continue
        l, b = radec_to_galactic(df["ra_deg"].to_numpy(),
                                 df["dec_deg"].to_numpy())
        lon, lat = galactic_mollweide(l, b)
        ax.scatter(lon, lat, s=s, color=color, alpha=alpha,
...
def main() -> int:
    ...
    for sid in (1, 2, 3):
        ...
        n = len(df)
        print(f"[H10] Stream {sid}: T1={n_t1:,} ({n_t1/n*100:.1f}%)  "
    ...
    gs = fig.add_gridspec(n_streams, 5, hspace=0.40, wspace=0.32,
    ...
    for r, (sid, df) in enumerate(sorted(streams.items())):
        ...
        _sky_panel(fig.add_subplot(gs[r, 4], projection="mollweide"),
                    df_t1, df_t2, df_t3, sid)
