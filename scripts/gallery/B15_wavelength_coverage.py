...
def main() -> int:
    ...
    ax.fill_between(SAMPLING_NM, p16, p84, color="#1f77b4", alpha=0.22,
                    label="Stream-1 16-84 percentile")
    ax.plot(SAMPLING_NM, median, "-", color="#1f77b4", lw=1.4,
            label="Stream-1 median Ye-corrected flux")

    ax.axvspan(SAMPLING_NM[0], SAMPLING_NM[-1], color="green", alpha=0.04,
               label="XP coverage 360-990 nm")
    ...
    for lam, label, family in sorted_lines:
        ...
        if lam - last_lambda < 12.0:
            ...
            cycle = 0
        y = y_positions[cycle]
        ax.text(lam, flux_max * y, label, color=color,
    ...
    # Family legend.
    handles_family = [plt.Line2D([0], [0], color=c, ls="--", lw=1.2,
                                  label=f"{f} lines")
                       for f, c in FAMILY_COLOR.items()]
    leg1 = ax.legend(loc="upper left", fontsize=8, framealpha=0.9)
    ax.add_artist(leg1)
    ax.legend(handles=handles_family, loc="lower right",
    ...
    ax.set_title("Gaia BP/RP XP coverage 360-990 nm with stellar absorption "
                 "features overlaid")
    ax.grid(alpha=0.25)
    ...
    for i, (label, lo, hi, central, family) in enumerate(PASSBANDS):
        ...
        ax.barh(i, hi - lo, left=lo, color=c, alpha=0.55,
                edgecolor=c, lw=1.0, height=0.75)
        ax.plot(central, i, "k|", ms=12, mew=1.5,
                label=r"$\lambda_\mathrm{eff}$" if i == 0 else None)
        ax.text(lo + (hi - lo) / 2, i, label, ha="center", va="center",
                fontsize=9, color="k",
                bbox=dict(facecolor="white", edgecolor="none",
                          alpha=0.85, pad=1.5))
    ...
    ax.text((SAMPLING_NM[0] + SAMPLING_NM[-1]) / 2, len(PASSBANDS) - 0.3,
            "Gaia BP/RP XP\n(360-990 nm,\n330 sampled points)",
            ha="center", va="bottom", fontsize=10, color="green",
            fontweight="semibold")
    # Vertical lines for the same absorption features
    for lam, label, family in sorted_lines:
        ax.axvline(lam, color=FAMILY_COLOR.get(family, "k"),
                   lw=0.6, ls=":", alpha=0.4)
    ...
    ax.set_title("Photometric input passband coverage "
                 "(Gaia BP/G/RP + 2MASS JHKs + AllWISE W1/W2)")
    ax.invert_yaxis()
