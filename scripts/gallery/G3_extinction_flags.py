...
def main() -> int:
    ...
    gs = fig.add_gridspec(
        2, 3, hspace=0.40, wspace=0.30, top=0.92, bottom=0.07, left=0.05, right=0.97
    )
    ...
    code_to_name = {0: "SFD/SF2011", 1: "Lallement+2022", 2: "Edenhofer+2024", 3: "nbhd fallback"}
    ...
    ax.barh(labels, counts.values, color="#3a6ea5", edgecolor="white", linewidth=1.4)
    ...
    if len(s):
        ...
        ax.axvline(
            0.3, color="#d62728", lw=1.6, ls="--", label=r"high-dispersion threshold $\sigma > 0.3$"
        )
        ax.legend(loc="upper right", fontsize=10)
