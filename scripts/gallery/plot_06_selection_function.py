...
def _load_omegas() -> pd.DataFrame:
    ...
    for c in ("p_parallax", "p_extinction", "p_ye", "p_ir",
             "omega_total", "selection_omega", "parallax_over_error"):
        if _have(schema, c):
            cols.append(c)
    ...
    if "omega_total" not in df.columns:
        df["omega_total"] = (
            df["p_ye"] * df["p_ir"] * df["p_parallax"] * df["p_extinction"]
        )
    return df
...
def omega_total_sky() -> None:
    ...
    sc = ax.scatter(x, y, c=np.clip(sub["omega_total"].to_numpy(), 0, 1),
                     cmap="viridis", s=2.5, alpha=0.75, rasterized=True, vmin=0, vmax=1)
    plt.colorbar(sc, ax=ax, shrink=0.65, pad=0.02, label=r"$\omega_{\rm total}$  (probability)")
    ax.set_title(rf"Compound selection function $\omega_{{\rm total}}$  —  Galactic coords  "
...
def omega_components() -> None:
    ...
    for ax, (col, title, cbar_lbl) in zip(axes.flat, comps):
        ...
        sc = ax.scatter(x, y, c=vals, cmap="viridis", s=2.5, alpha=0.75,
    ...
    fig.suptitle(r"$\omega$ decomposed across its four components  —  Galactic coords  "
...
def omega_vs_g() -> None:
    ...
    ax.axhline(df["omega_total"].mean(), color="#333", lw=0.8, ls="--",
...
def omega_histograms() -> None:
    ...
    for col, color in [("p_ye", "#2ca02c"), ("p_ir", "#1f77b4"),
                       ("p_parallax", "#ff7f0e"), ("p_extinction", "#9467bd")]:
        axes[0].hist(df[col].dropna(), bins=bins, histtype="step",
                     lw=1.5, color=color, label=f"{col}  (mean={df[col].mean():.2f})")
    ...
    axes[1].hist(df["omega_total"].dropna(), bins=bins, color="#d62728",
                  edgecolor="#333", alpha=0.85)
    axes[1].axvline(df["omega_total"].mean(), color="k", lw=1, ls="--",
