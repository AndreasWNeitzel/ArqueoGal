...
def _load() -> "pd.DataFrame":
    ...
    want = ["source_id", "ra_deg", "dec_deg", "b_deg", "g_mag",
            "r_med_photogeo", "distance_pc",
...
def av_map_stack() -> None:
    ...
    panels = [
        ("av_edenhofer",   r"Edenhofer+2024  —  3D, truncates at $d=1.25$ kpc",
         "depth: 0 to min(d, 1.25) kpc"),
        ("av_lallement",   r"Lallement+2022  —  3D, truncates at $d=3$ kpc",
         "depth: 0 to min(d, 3) kpc"),
        ("av_sfd",         r"SFD 1998  —  2D asymptotic (full LOS to $\infty$)",
         "depth: full LOS  (→ highest values in disc by design)"),
        ("av_nbhd_median", r"GSP-Phot 75 pc nbhd-median (fallback)",
         "depth: per-star GSP-Phot out to infinity, median over 75 pc sphere"),
    ]
    x_all, y_all = radec_to_galactic_mollweide(sub["ra_deg"].to_numpy(),
                                                sub["dec_deg"].to_numpy())
    ...
    for ax, (col, title, depth_note) in zip(axes.flat, panels):
        ...
        if n_valid == 0:
            ...
            ax.text(0, 0, "0 stars with valid " + col + "\n(Stream 3 is RGB at d > 1.25 kpc;\n"
        ...
        sc = ax.scatter(x_all[valid], y_all[valid],
                        c=np.clip(vals[valid], 0, 3), cmap="viridis",
                        s=0.6, alpha=0.7, rasterized=True, vmin=0, vmax=3)
    ...
    fig.suptitle(r"Stream 3 — four-source $A_V$ stack  (Galactic coords)  —  "
...
def av_source_breakdown() -> None:
    ...
    axes[1].remove()
    colors_by_src = {str(k): PALETTE.get(str(k), f"C{i}")
                     for i, k in enumerate(counts.index.astype(str))}
    ...
    for k, color in colors_by_src.items():
        ...
        if m.sum() == 0:
            continue
        x, y = radec_to_galactic_mollweide(sub.loc[m, "ra_deg"].to_numpy(),
                                           sub.loc[m, "dec_deg"].to_numpy())
        ax_map.scatter(x, y, s=0.5, alpha=0.4, color=color, label=f"{k} (n={m.sum():,})",
                       rasterized=True)
...
def av_histograms() -> None:
    ...
    for col, label, color in [
        ...
        if len(vals) == 0:
            continue
        ax.hist(np.clip(vals, 0, 3), bins=bins, histtype="step",
                lw=1.5, color=color, label=f"{label}  (n={len(vals):,}, "
                                            f"med={np.median(vals):.2f})")
    ax.set_xlabel(r"$A_V$ [mag]  (clipped at 3)")
    ax.set_ylabel("count")
    ax.set_yscale("log")
    ax.set_title(r"Stream 3 $A_V$ distributions across the four extinction sources  "
...
def av_edenhofer_vs_lallement() -> None:
    ...
    df = pq.read_table(
        ...
        columns=["av_edenhofer", "av_lallement", "av_sfd", "av_nbhd_median",
                 "r_med_photogeo"],
    ).to_pandas().dropna(subset=["av_edenhofer", "av_lallement"])
    ...
    hb0 = axes[0].hexbin(sub["av_edenhofer"], sub["av_lallement"], gridsize=60,
                         cmap="viridis", bins="log", extent=(0, 2, 0, 2), mincnt=1)
    ...
    hb1 = axes[1].hexbin(sub["distance_pc"] / 1000, delta, gridsize=60,
                         cmap="plasma", bins="log", extent=(0, 4, -1, 1), mincnt=1)
    ...
    axes[1].set_xlim(0, 4); axes[1].set_ylim(-1, 1)
