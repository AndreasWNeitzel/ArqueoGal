...
ELEMENT_LABELS = {"teff": "Teff", "logg": "log g", "mh": "[M/H]",
...
def _render(parquet: Path, stream_label: str, out_png: Path) -> None:
    if not parquet.exists():
        return
    cols = (["g_mag", "r_med_photogeo", "release_tier"]
...
def _do_render(df: pd.DataFrame, stream_label: str, out_png: Path) -> None:
    ...
    if "g_mag" in df.columns:
        ...
        stacks = _stack_by_bin(df["g_mag"].to_numpy(),
                                 df["release_tier"].to_numpy(), bins)
        centres = 0.5 * (bins[1:] + bins[:-1])
        bottom = np.zeros_like(centres)
        colors = ("#2ca02c", "#ff7f0e", "#d62728")
        for ti, color, label in zip(range(3), colors, ("Tier 1", "Tier 2", "Tier 3")):
            axes[0, 0].fill_between(centres, bottom, bottom + stacks[ti], color=color,
                                       alpha=0.7, label=label, step="mid")
            bottom += stacks[ti]
        axes[0, 0].set_xlabel("G (mag)"); axes[0, 0].set_ylabel("% per G bin")
    ...
    if "r_med_photogeo" in df.columns:
        ...
        stacks = _stack_by_bin((df["r_med_photogeo"].to_numpy() / 1000.0),
                                 df["release_tier"].to_numpy(), bins)
        centres = 0.5 * (bins[1:] + bins[:-1])
        bottom = np.zeros_like(centres)
        colors = ("#2ca02c", "#ff7f0e", "#d62728")
        for ti, color, label in zip(range(3), colors, ("Tier 1", "Tier 2", "Tier 3")):
            axes[0, 1].fill_between(centres, bottom, bottom + stacks[ti], color=color,
                                       alpha=0.7, label=label, step="mid")
            bottom += stacks[ti]
    ...
    for ti, color, label in zip((1, 2, 3), ("#2ca02c", "#ff7f0e", "#d62728"),
    ...
    for b, r in zip(bars, rates):
        axes[1, 1].text(b.get_x() + b.get_width() / 2, r + 0.3, f"{r:.1f}%",
                          ha="center", fontsize=8)

    fig.suptitle(f"Release-tier regime composition ({stream_label} hybrid release, schema v5)",
                  fontsize=11)
    fig.tight_layout()
