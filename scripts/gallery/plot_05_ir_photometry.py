...
def ir_coverage_sky() -> None:
    ...
    sub = df.iloc[idx]
    have_all = np.isfinite(sub[["j_mag", "h_mag", "k_mag", "w1_mag", "w2_mag"]].to_numpy()).all(
        axis=1
    )
    ...
    fig.suptitle(
        f"Stream 3 IR coverage across the sky  —  Galactic coords  "
...
def ir_color_color() -> None:
    ...
    ax.hexbin(jk, w12, gridsize=80, cmap="viridis", bins="log", extent=(-0.2, 1.8, -0.5, 0.7))
