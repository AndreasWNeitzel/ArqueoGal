...
def _draw(ax, x, y, *, title, n):
    ...
    hb = ax.hexbin(
        x[ok], y[ok], gridsize=80, extent=(-1.6, 0.55, -0.05, 0.42),
    ...
    ax.axhline(0.15, color=PALETTE["accent"], ls="--", lw=1.6, alpha=0.8,
...
def main() -> int:
    ...
    _draw(axes[0], df["mh_pred"].to_numpy(), df["alpha_m_pred"].to_numpy(),
          title="OURS, XP→MLP predictions", n=n)
    _draw(axes[1], df["mh_apogee"].to_numpy(), df["alpha_m_apogee"].to_numpy(),
