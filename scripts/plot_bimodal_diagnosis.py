...
def main() -> None:
    ...
    tr = pd.read_parquet(
        TRAIN, columns=["teff_apogee", "logg_apogee", "mh_apogee", "alpha_m_apogee"]
    )
    ...
    d = pr.loc[rel]
    stripe = d["alpha_m_pred"].between(0.105, 0.120) & d["mh_pred"].between(-1.5, -0.3)
    ...
    ax_top.hist(
        ...
        color="0.30",
        lw=1.2,
        label=f"Stream-1 training  (all, N={len(a_all):,})",
    )
    ax_top.axvline(
        np.median(a_cell),
        color="C3",
